...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    combined = list(zip(support_inputs, support_labels))
    np.random.shuffle(combined)
    support_inputs[:], support_labels[:] = zip(*combined) if combined else ([], [])
    
    if prompt_mode == 'qa':
        for inp, lbl in zip(support_inputs, support_labels):
            prompt += f"{inp} In the {lbl}. "
        prompt += f"{test_input} In the"
    elif prompt_mode == 'none':
        prompt = " ".join([f"{inp} {lbl}" for inp, lbl in zip(support_inputs, support_labels)])
        prompt += f" {test_input}"
    elif prompt_mode == 'tldr':
        prompt = " ".join([f"{inp} TL;DR: {lbl}" for inp, lbl in zip(support_inputs, support_labels)])
        prompt += f" {test_input} TL;DR:"
    elif prompt_mode == 'custom':
        prompt = " ".join([f"###Original article\n{inp}.\n\n ###Summary\n{lbl}\n\n\n" for inp, lbl in zip(support_inputs, support_labels)])
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    with torch.inference_mode():
        for _ in range(max_tokens):
            ...
            current_input_ids = torch.cat([current_input_ids, torch.tensor([[next_token_id]], device=model.device)], dim=-1)
