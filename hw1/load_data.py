...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        classes = random.sample(self.folders, self.num_classes)
        
        image_batch = np.zeros((self.num_samples_per_class, self.num_classes, self.dim_input))
        label_batch = np.zeros((self.num_samples_per_class, self.num_classes, self.num_classes))
        
        for i, class_folder in enumerate(classes):
            image_paths = random.sample(os.listdir(class_folder), self.num_samples_per_class)

            for j, image_path in enumerate(image_paths):
                image_file = os.path.join(class_folder, image_path)
                image = self.image_file_to_array(image_file, self.dim_input)
                image_batch[j, i, :] = image

                label_batch[j, i, :] = np.eye(self.num_classes)[i]

        order = np.arange(self.num_classes)
        np.random.shuffle(order)

        image_batch = image_batch[:, order, :]
        label_batch = label_batch[:, order, :]
        
        ret1, ret2 = torch.tensor(image_batch, dtype=torch.float32, device=self.device), torch.tensor(label_batch, dtype=torch.float32, device=self.device)
