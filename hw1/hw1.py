...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        labels_with_zeros = torch.zeros_like(input_labels)
        labels_with_zeros[:, :-1, :, :] = input_labels[:, :-1, :, :]
        
        batch_size, seq_len, num_classes, img_size = input_images.shape
        input_images = input_images.reshape(batch_size, -1, img_size)
        labels_with_zeros = labels_with_zeros.reshape(batch_size, -1, num_classes)

        combined_input = torch.cat([input_images, labels_with_zeros], dim=-1)
        
        # LSTM 레이어를 통과
        x, _ = self.layer1(combined_input)
        x, _ = self.layer2(x)
        
        # 소프트맥스를 통해 확률로 변환
        return F.softmax(x, dim=-1).view(batch_size, seq_len, num_classes, num_classes)
        #############################
    ...
    def loss_function(self, preds, labels):
        ...
        query_labels = query_labels.argmax(dim=-1)
        
        # 크로스 엔트로피 손실 계산
        return F.cross_entropy(query_preds.reshape(-1, self.num_classes), query_labels.view(-1))
        #############################
...
def main(config):
    ...
    writer = SummaryWriter(
        f"runs/{config.num_classes}_{config.num_shot}_{config.random_seed}_{config.hidden_dim}_{config.learning_rate}_{config.hidden_dim}"
    )
