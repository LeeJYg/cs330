...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps):
            class_score = self._forward(images, parameters)
            accuracies.append(util.score(class_score, labels))
            loss = F.cross_entropy(class_score, labels)
            
            gradient = torch.autograd.grad(loss, inputs=list(parameters.values()), create_graph=True)
            
            for parameter_name, grad in zip(parameters.keys(), gradient):
                parameters[parameter_name] = parameters[parameter_name] - self._inner_lrs[parameter_name] * grad
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            loss = F.cross_entropy(class_scores, labels_query)
